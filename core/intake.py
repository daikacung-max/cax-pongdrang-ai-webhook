...
def _requests_intake(question, text):
    ...
    if any(_contains_phrase(text, marker) for marker in explicit_markers):
        return True
...
def assess(question, history):
    ...
    if "identity_card_reissue" in by_code:
        ...
        chosen = by_code["assault_evidence"]
    elif any(x in text for x in ("bi trom", "bi de doa", "de doa", "to giac", "trinh bao toi pham", "tin bao toi pham")) and "crime_report" in by_code:
        chosen = by_code["crime_report"]
    elif any(x in text for x in ("mat dien thoai", "mat xe", "mat tai san", "mat giay to", "that lac")) and "lost_document" in by_code:
        chosen = by_code["lost_document"]
    else:
