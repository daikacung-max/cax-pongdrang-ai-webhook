...
PLAN_SCHEMA = {
    ...
    "properties": {
        ...
        "explicit_references": {"type": "array", "items": {"type": "object", "properties": {"law_hint": {"type": "string"}, "article": {"type": "string"}}, "required": ["law_hint", "article"], "additionalProperties": False}, "maxItems": 4},
        "needs_clarification": {"type": "boolean"},
        "clarification_question": {"type": ["string", "null"]},
        "complexity": {"type": "string", "enum": ["simple", "complex"]},
        "complexity_reasons": {"type": "array", "items": {"type": "string"}, "maxItems": 3},
    },
    "required": ["is_legal", "search_queries", "explicit_references", "needs_clarification", "clarification_question", "complexity", "complexity_reasons"],
...
def _norm(text):
    ...
    return _fix_common_typos(re.sub(r"\s+", " ", text).strip())


SEARCH_VOCAB = [
    (["bi danh", "danh nguoi", "nguoi khac danh", "bi nguoi khac danh", "bi thuong", "thuong tich", "dung dao", "hung khi"], "Tội cố ý gây thương tích hoặc gây tổn hại cho sức khỏe của người khác"),
...
def quick_plan(question):
    q = _norm(question)
    refs = [{"law_hint": "", "article": m.group(1)} for m in re.finditer(r"\bdieu\s+(\d+[a-z]?)\b", q)]
    queries = [expansion for phrases, expansion in SEARCH_VOCAB if any(p in q for p in phrases)] + [question]
    reasons = ["multiple_explicit_articles"] if len(refs) > 1 else []
    return {"is_legal": any(x in q for x in LEGAL_HINTS), "search_queries": queries[:4], "explicit_references": refs[:4], "needs_clarification": False, "clarification_question": None, "complexity": "complex" if reasons else "simple", "complexity_reasons": reasons}
...
def _sanitize_plan(candidate, baseline, contextual):
    ...
    for value in list(candidate.get("search_queries") or []) + list(baseline.get("search_queries") or []):
        ...
        if value and value not in queries: queries.append(value[:220])
    result["search_queries"] = queries[:4]
    explicit = set(re.findall(r"\bdieu\s+(\d+[a-z]?)\b", _norm(contextual)))
    refs = []
    for item in candidate.get("explicit_references") or []:
        article = str((item or {}).get("article") or "").lower().strip()
        if article in explicit: refs.append({"law_hint": str((item or {}).get("law_hint") or "")[:120], "article": article})
    ...
    followup = candidate.get("clarification_question")
    result["clarification_question"] = str(followup).strip()[:280] if followup else None
    result["complexity"] = candidate.get("complexity") if candidate.get("complexity") in ("simple", "complex") else baseline["complexity"]
    result["complexity_reasons"] = [str(x)[:120] for x in candidate.get("complexity_reasons") or []][:3]
    return result
